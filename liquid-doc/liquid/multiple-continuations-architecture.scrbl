...
@title[#:style '(toc)]{Multiple Execution Path Exits Architecture}
...
@section[#:tag "OfPCFA"]{Option for Passing Continuation Function Arguments}
...
When a library programmer defines a function that accepts continuations to handle results
...
One approach was to define a new syntax object to define functions, one where
...
would have to define keywords for arguments they typically do not use.

In an alternative approach, continuations are provided to the function definition syntax
...
to make sure that arguments are always visible.  This could be a viable approach.

As a third option the library routine could pack up everything that could possibly
...
@verbatim|{
  ...
  (x-hash-ref (Λ table a)
    (Λ
      (cons (λ(v) (display "found") (displayln v))  '(result))
      (cons (λ(k) (display "no such key: ") (displayln k))  '(key))
    ))
  ...
  }|
...
was not necessary to provide this because @racket[continue-ok] is called with the look-up
...
I find it interesting that the use of specifiers for the continuation argument forces the
...
of the entire args list,  and the same for conts, then we can bury the @racket[match-let]
...
We Will not get to see the arguments for the continuation,  i.e. @racket[(cdr cont)]
until run time, so we cannot build the eval with a call like this with a define-syntax function.
...
values may not have been created at the time of the call to the continuation.  Rather @racket[returns]
must be those guaranteed to be available to this specific continuation.
...
a distinction that we now say it in the context of named argument lists via @racket[mc:define].
