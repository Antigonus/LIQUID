...
@subsection{token}
   ...
   types.  Typical query languages do not do this.  Also, each token must have either or
   both of'at:source' or 'at:source-nds' attributes.  The at:source attribute provides an
   interval of character positions in the source stream where the token was parsed from.
   The at:source-nds attribute provides a list of nodes that were used in creating the
   given node.
